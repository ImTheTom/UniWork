...
while (~done) 
  ...
  Jsur(iter) = logisticCost(obj, X, Y, reg); % compute cost at this iteration
  ...
  for j=1:n,
    ...
    % Compute gradient:
    grad = X1(j,:)*(sigma-Y(j))+2*theta*reg;
    
    obj.wts = obj.wts - step * grad;      % take a step down the gradient
  end;

  JDel =logisticCost(obj, X, Y, reg); % compute jdelta
  if iter==stopIter || abs(JDel-Jsur(iter))<stopTol % check condition
    done = true;
  end
