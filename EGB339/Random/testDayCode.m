...
for i = 1:3
    ...
    angles = FindAngles(tempIntialLocation(1),tempIntialLocation(2),20);
    fprintf('angles are %d %d %d %d', angles(1),angles(2),angles(3),angles(4));
    m=input('Do you want to continue, Y/N:','s');
    if m=='Y'
        claw.setJointPosition(claw.BASE, angles(1));
        pause(2)
        claw.setJointPosition(claw.SHOULDER, angles(2));
        pause(2)
        claw.setJointPosition(claw.ELBOW, angles(3));
        pause(2)
        claw.setJointPosition(claw.WRIST, angles(4));
        pause(2)
    else
        fprintf('\ngoing past this point\n')
    end
    claw.setJointPosition(claw.SHOULDER, movingAngles(2));
    pause(2)
    claw.setJointPosition(claw.ELBOW, movingAngles(3));
    pause(2)
    claw.setJointPosition(claw.WRIST, movingAngles(4));
    pause(2)
    fprintf('The shape located at X: %dmm Y: %dmm from the arms final positons\n', tempFinalLocation(1),tempFinalLocation(2));
    angles = FindAngles(tempFinalLocation(1),tempFinalLocation(2),20);
    fprintf('angles are %d %d %d %d', angles(1),angles(2),angles(3),angles(4));
